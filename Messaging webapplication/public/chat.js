...
window.setInterval(UserChat, 2000)
...
function UserChat(){
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if(messages.length >0){
                ...
                for(let i = 0; i < messages.length; i++){
                    allMessages += `<div><div style="background: rgb(7, 124, 213);">${messages[i].name}</div><div >${messages[i].Message}</div></div>`
                }
                //Add users to page.
    ...
    xhttp.open("GET", "/allMessages", true);
...
function sendingMessage(){
    ...
    //Send new user data to server
    xhttp.open("POST", "/sendTheMsg", true);
    xhttp.setRequestHeader("Content-type", "application/json");
